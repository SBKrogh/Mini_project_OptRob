...
M = 50; % The duration of the control process %200
...
% Specify the optimization of cost %%% FILL IN %%% 
     minimize( -(P_E(k:k+L-1)'*Q_E-(P_G(k:k+L-1)'*Q_G + P_W(k:k+L-1)'*Q_W))*Ts)
...
figure
stairs(E_A_sys)
title('The state of charge in the accumulator(E_A)')
ylabel('[MWh]')
xlabel('Sample [hour]')
 
figure
stairs(Q_W_sys)
title('Power from using waste(Q_W)')
ylabel('[MW]')
xlabel('Sample [hour]')
 
figure
stairs(Q_G_sys)
title('Power from unsing gas(Q_G)')
ylabel('[MW]')
xlabel('Sample [hour]')
