...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
variables Q_E(k)     % Power leaving plant                      |
variables Q_G(k)     % Power from gas turbine                   |
variables Q_W(k)     % Power from waste burner                  |
variables Q_A_in(k)  % Power-fed to accumulator                 |
variables Q_A_out(k) % Power leaving accumulator                |
variables Q_bp(k)    % Power bypass accumulator                 |
variables E_A(k)     % Energy stored in accumulator             |
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Specify the optimization of cost %%% FILL IN %%%              |
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
maximize(sum(P_E(k)*Q_E(k) - (P_G(k)*Q_G(k) + P_W(k)*Q_W(k)))*Ts)
...
Q_A_in(k) >= Q_A_in_min;
Q_A_in(k) <= Q_A_in_max;
Q_A_out(k) >= Q_A_out_min;
Q_A_out(k) <= Q_A_out_max;

Q_W(k) + Q_G(k) == Q_bp(k) + Q_A_in(k);
Q_E(k) == Q_bp(k) + Q_A_out(k);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
E_A(k+1) = E_A(k) + (Q_A_in(1) - Q_A_out(1))*Ts;
